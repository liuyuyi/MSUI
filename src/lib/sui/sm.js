...
function ($) {
    ...
    $.openModal = function (modal, cb) {
        ...
        // if (modal.hasClass('modal-out')) modal.removeClass('modal-out');
        // if(modal.css('display') === 'block') return;
    ...
    $.closeModal = function (modal) {
        ...
        if (isPopup) {
            if (modal.length === $('.popup.modal-in').length) {
                overlay.removeClass('modal-overlay-visible').remove();
            }
        ...
        // modal.removeClass('modal-in').addClass('modal-out').hide();
...
function ($) {
    ...
    MultiSelect.prototype = {
        ...
        checkInitItem: function (name) {

            var self = this,
                initItems = self.chooseSelectArr,
                len = initItems.length;
        ...
        showModal: function () {
            ...
            for (var i = 0, len = select.length; i < len; i++) {
                ...
                var id = new Date().getTime() + '_' + i,
                    name = select[i],
                    checked = self.checkInitItem(name) ? 'checked' : '';

                liHtml += '<li>\
                                <div class="label-checked-box">\
                                        <input type="checkbox" id="check-' + id + '" ' + checked + ' value="' + name + '">\
                                        <label for="check-' + id + '">\
            ...
            shtml = '<div class="modal diy-modal modal-out" style="display:block">\
            ...
            $('.diy-modal').trigger('open');
            $('.diy-modal').removeClass('modal-out').addClass('modal-in');

            $('.diy-modal input').off().on('change', function () {
            ...
            $('.modal-overlay-visible').on('click', function () {
                ...
                $('.diy-modal').addClass('modal-out');
                $('.diy-modal').trigger('close');
                $(this).off().removeClass('modal-overlay-visible');
                $('.diy-modal input').off();
                $('.diy-modal').remove();
        ...
        getCheckedItem: function () {
            ...
            var self = this,
                $elem = self.opt.elem,
                inputChecked = $('.diy-modal input:checked'),
                len = inputChecked.length,
                shtml = '',
                title = self.opt.title;

            self.chooseSelectArr = [];

            for (var i = 0; i < len; i++) {

                var name = $(inputChecked[i]).val();

                shtml += '<span class="lable">' + name + ' <i class="iconfont icon-shanchu" data-name="' + name + '"></i></span>';

                self.chooseSelectArr.push(name);

            }

            $elem.attr('data-items', self.chooseSelectArr);

            $('.multi-select-list').empty().append(shtml);

            if (shtml === '') {
                $('.multi-select-list').text(title);
            }

            $('.multi-select-list').off().on('click', '.icon-shanchu', function () {

                $(this).parent('.lable').remove();
                ...
                for (var i = 0, len = items.length; i < len; i++) {
                    ...
                    if (items[i] !== thisItem) {
                        self.chooseSelectArr.push(items[i]);
                    }

                }
                $elem.attr('data-items', self.chooseSelectArr);

                if (self.chooseSelectArr.length === 0) {
                    $('.multi-select-list').text(title);
                }
    ...
    $.fn.multiSelect = function (params) {
        return this.each(function () {
            ...
            var p = {};
            p.elem = $this;
            p.select = ['X3', 'Q5', 'X3M', 'X3C', 'Q5S', 'X3T', 'Q5C', 'VP1000U'];
            p.title = '选择设备型号';

            multiSelect = new MultiSelect($.extend(p, params));
            multiSelect.init();
...
function ($) {
    ...
    var Picker = function (params) {
        ...
        function openOnInput(e) {
            ...
            // if ($('.picker-columns').length > 1) return
            // 安卓微信webviewreadonly的input依然弹出软键盘问题修复
            if ($.device.isWeixin && $.device.android && p.params.inputReadOnly) {
                /*jshint validthis:true */
                this.focus();
            ...
            if (p.opened || $('.picker-columns').length > 0) {
                return;
            }
            p.open();
            if (p.params.scrollToInput) {
...
function ($) {
    ...
    var changeCols = function (format) {
    ...
    var defaults = {
        ...
        value: [today.getFullYear(), formatNumber(today.getMonth() + 1), formatNumber(today.getDate()), today.getHours(), formatNumber(today.getMinutes()), formatNumber(today.getSeconds())],
        ...
        formatValue: function (p, values, displayValues) {
            ...
            if (format === 'YYYY-MM-DD') {
                ...
                value = displayValues[0] + '-' + values[1] + '-' + values[2];

            } else if (format === 'YYYY-MM-DD hh:mm:ss') {

                value = displayValues[0] + '-' + values[1] + '-' + values[2] + ' ' + values[3] + ':' + values[4] + ':' + values[5];

            } else if (format === 'YYYY-MM-DD hh:mm') {

                value = displayValues[0] + '-' + values[1] + '-' + values[2] + ' ' + values[3] + ':' + values[4];
        ...
        onOpen: function (p) {
            ...
            $(p.input).val(p.params.formatValue(p, p.value, p.value));
